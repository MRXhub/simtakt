...
def main() -> int:
    # Keep both adapter output and the registered package in a disposable
    # workspace directory.  The registry record is transient as well: this
    # demo must not contend with the runtime monitor's shared .runtime tree or
    # leave generated files in the example.
    record_path = ROOT / "records" / "artifacts" / f"{PACKAGE_ID}.json"
    with tempfile.TemporaryDirectory(
        dir=ROOT, prefix=".materialize-demo-"
    ) as temporary_root:
        package_dir = Path(temporary_root) / "materialized-package"
        resolved = resolve_adapter(ROOT, "minimal-simulation")
        resolved.adapter.package_dir = Path(temporary_root) / "adapter-packages"
        ...
        try:
            ...
            checked = resolve_workspace_artifact(
                ROOT, PACKAGE_ID, expected_kind="input-package"
            )
